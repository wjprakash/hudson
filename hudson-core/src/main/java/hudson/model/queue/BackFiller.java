...
public class BackFiller extends LoadPredictor {
    ...
    public Iterable<FutureLoad> predict(MappingWorksheet plan, ComputerExt computer, long start, long end) {
        ...
        for (BuildableItem bi : HudsonExt.getInstance().getQueue().getBuildableItems()) {
    ...
    private static final class PseudoExecutorSlot extends ExecutorSlot {
        private ExecutorExt executor;

        private PseudoExecutorSlot(ExecutorExt executor) {
    ...
    private TentativePlan makeTentativePlan(BuildableItem bi) {
        ...
        try {
            ...
            Mapping m = HudsonExt.getInstance().getQueue().getLoadBalancer().map(bi.task, worksheet);
